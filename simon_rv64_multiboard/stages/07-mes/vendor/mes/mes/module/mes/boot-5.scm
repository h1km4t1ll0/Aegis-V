...
(letrec ((expand (lambda (clauses)
                   (if (null? clauses) (list)
                       (if (or (eq? (car (car clauses)) (quote else))
                               (defined? (car (car clauses))))
                           (cdr (car clauses))
                           (expand (cdr clauses)))))))
  (module-define! the-root-module (quote cond-expand-expander) expand)
  (module-define! the-root-module (quote else) #t)
  (module-define! the-root-module (quote mes) #t))
